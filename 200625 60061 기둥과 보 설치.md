...
def solution(n, build_frame):
    def check_type(x, y, a):
        if a:
            if pillar[y-1][x] or pillar[y-1][x+1] or (x > 0 and beam[y][x-1] and beam[y][x+1]):
                return 1
        else:
            if y == 0 or (x > 0 and beam[y][x-1]) or beam[y][x] or pillar[y-1][x]:
                return 1
        return 0
    ...
    for x, y, a, b in build_frame:
        ...
        # 삭제
            ...
            if a:
                beam[y][x] = 0
                if x > 0 and beam[y][x-1] and not check_type(x-1, y, a):
                    beam[y][x] = 1
                elif x < n+1 and beam[y][x+1] and not check_type(x+1, y, a):
                    beam[y][x] = 1
                elif pillar[y][x] and not check_type(x, y, 0):
                    beam[y][x] = 1
                elif pillar[y][x+1] and not check_type(x+1, y, 0):
                    beam[y][x] = 1
            # 기둥
            else:
                pillar[y][x] = 0
                if x > 0 and beam[y+1][x-1] and not check_type(x-1, y+1, 1):
                    pillar[y][x] = 1
                elif x < n+1 and beam[y+1][x] and not check_type(x, y+1, 1):
                    pillar[y][x] = 1
                elif pillar[y+1][x] and not check_type(x, y+1, a):
                    pillar[y][x] = 1
    # check pillar and beam
    for i in range(n+1):
        for j in range(n+1):
            if pillar[j][i]:
                answer.append([i, j, 0])
